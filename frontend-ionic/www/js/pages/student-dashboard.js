...
Pages['student-dashboard'] = {
  ...
  render() {
    ...
    return `
      <ion-header><ion-toolbar>
        ...
        <ion-title id="dash-title" style="text-align:center;font-size:17px;font-weight:700;color:var(--bgi-text);">Student Dashboard</ion-title>
  ...
  _syncNotificationBadge(count) {
    const tile = document.getElementById('tile-notifications');
    if (!tile) return;
    const badge = tile.querySelector('.notif-badge');
    ...
    return `
      <div style="background:linear-gradient(135deg, rgba(10,77,173,0.08), rgba(34,197,94,0.08));border:1px solid rgba(10,77,173,0.16);border-radius:16px;padding:14px 14px 12px;margin-bottom:14px;">
        ...
        <div style="display:grid;gap:8px;font-size:11px;color:var(--bgi-text-secondary);">
          ${qrUrl ? `<div style="display:flex;justify-content:center;margin:4px 0 6px;"><img src="${UI.escapeHtml(qrUrl)}" alt="Approved QR Pass" style="width:140px;height:140px;object-fit:contain;background:#fff;border-radius:12px;padding:6px;border:1px solid rgba(10,77,173,0.15);" /></div>` : ''}
  ...
  _showApplyLeaveModal() {
        ...
        <div style="max-width:480px;margin:0 auto;">
          ...
          <div style="background:var(--bgi-surface);border-radius:12px;border:1px solid var(--bgi-border);padding:8px 14px 10px;margin-bottom:10px;">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
              <div>
                <label style="font-size:10px;font-weight:700;color:var(--bgi-text-secondary);display:block;padding-top:2px;letter-spacing:0.5px;text-transform:uppercase;">Student Name</label>
                <ion-input id="student-name" value="${UI.escapeHtml(currentUser.name || '')}" style="font-size:13px;font-weight:500;color:var(--bgi-text);--padding-top:2px;--padding-bottom:2px;"></ion-input>
              </div>
              <div>
            ...
            <div style="margin-top:8px;">
              <label style="font-size:10px;font-weight:700;color:var(--bgi-text-secondary);display:block;padding-top:2px;letter-spacing:0.5px;text-transform:uppercase;">TG Name</label>
              <ion-input id="tg-name" placeholder="Enter TG / mentor name" style="font-size:13px;font-weight:500;color:var(--bgi-text);--padding-top:2px;--padding-bottom:2px;"></ion-input>
            </div>
          </div>
          ...
          <div style="margin-bottom:8px;">
            <div style="background:var(--bgi-surface);border-radius:12px;border:1px solid var(--bgi-border);padding:2px 14px 4px;">
              <div style="display:flex;justify-content:space-between;align-items:center;">
                <label style="font-size:10px;font-weight:700;color:var(--bgi-text-secondary);display:block;padding-top:4px;letter-spacing:0.5px;text-transform:uppercase;">Date <span style="color:var(--bgi-danger);">*</span></label>
    ...
    // entry/exit time fields removed — leave uses date only
    ...
    modal.querySelector('#apply-submit-btn').addEventListener('click', async () => {
      ...
      const studentName = modal.querySelector('#student-name').value?.trim();
      const enrollmentNumber = modal.querySelector('#enrollment-number').value?.trim();
      const branch = modal.querySelector('#student-branch').value?.trim();
      const semester = modal.querySelector('#student-semester').value?.trim();
      const tgName = modal.querySelector('#tg-name').value?.trim();
      const leaveDate = modal.querySelector('#leave-date').value;
      const errorEl = modal.querySelector('#apply-error');

      if (!reason) { 
        errorEl.style.display = 'block'; 
        errorEl.textContent = 'Please enter the purpose of leave'; 
        return; 
      }
      if (!leaveDate) { 
        errorEl.style.display = 'block'; 
      ...
      try {
        const fields = {
          ...
          enrollmentNumber: enrollmentNumber || currentUser.rollNumber || currentUser.profile?.roll_number || '',
          branch: branch || currentUser.branch || currentUser.profile?.branch || '',
          semester: semester || currentUser.semester || currentUser.profile?.semester || '',
          tgName,
      ...
      body.innerHTML = `
        <div style="text-align:center;padding:10px 0 4px;">
          <div style="position:relative;display:inline-block;">
            <div style="width:90px;height:90px;border-radius:50%;background:rgba(var(--bgi-primary-rgb),0.1);display:flex;align-items:center;justify-content:center;overflow:hidden;border:3px solid var(--bgi-primary);margin:0 auto;">
              ${photoUrl ? 
                `<img src="${UI.escapeHtml(photoUrl)}" style="width:100%;height:100%;object-fit:cover;" id="profile-photo-img" />` :
                `<ion-icon name="person" style="font-size:45px;color:var(--bgi-primary);"></ion-icon>`
              }
            </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:14px;">
          ...
          <ion-button expand="block" fill="outline" id="change-password-btn" style="font-size:12px;height:40px;--border-radius:10px;">
            <ion-icon name="key-outline" slot="start" style="font-size:14px;"></ion-icon> Password
          </ion-button>
        </div>

        <ion-button expand="block" fill="outline" color="danger" class="mt-8" id="logout-btn" style="font-size:12px;height:40px;--border-radius:10px;">
        ...
        <div id="photo-upload-overlay" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:9999;display:none;align-items:center;justify-content:center;flex-direction:column;gap:12px;">
          ...
          <p style="color:#fff;font-size:14px;font-weight:500;">Uploading photo...</p>
        </div>
      `;
  ...
  async _handleProfilePhotoUpload(file) {
    ...
    try {
      ...
      const data = await Api.putMultipart('/auth/profile-photo', {}, compressedFile, 'photo');
      ...
      if (user) {
        ...
        if (user.profile) user.profile.photo = user.profileImageUrl;
  ...
  _showEditProfileModal(user) {
    ...
    modal.innerHTML = `
      ...
      <ion-content class="ion-padding">
        <ion-list lines="none" style="padding:0;">
          ...
          <ion-item style="border:1px solid var(--bgi-border);border-radius:10px;margin-bottom:8px;--min-height:44px;">
            <ion-label position="stacked" style="font-size:11px;font-weight:600;color:var(--bgi-text-secondary);">Branch</ion-label>
            <ion-input id="edit-branch" value="${UI.escapeHtml(user.branch || user.profile?.branch || '')}" style="font-size:13px;"></ion-input>
          </ion-item>
          <ion-item style="border:1px solid var(--bgi-border);border-radius:10px;margin-bottom:8px;--min-height:44px;">
            <ion-label position="stacked" style="font-size:11px;font-weight:600;color:var(--bgi-text-secondary);">Semester</ion-label>
    ...
    modal.querySelector('#edit-profile-save').addEventListener('click', async () => {
      ...
      try {
        const payload = { name, phone, campus, college, department, branch, rollNumber, semester: semester ? parseInt(semester, 10) : undefined };
        Object.entries(payload).forEach(([key, value]) => { if (!value && value !== 0) delete payload[key]; });
